...
### Resume the simulation from the 6th rep

```{r}
n_sim <- 10

.Random.seed <- rng_states_serial[[6]]

tic("Serial simulation")
rng_states_serial2 <- list() #Store the state of the random number generator 

results <- foreach( i = 1:n_sim, .combine="rbind") %do% {
  rng_states_serial2[[i]] <- .Random.seed  # save RNG state
  data <- simple_sccs_data(n = 1000, obs_time = 500, baseline_rate = 1e-5, beta=log(2), risk_length=28)
  result<-analyse_sccs_simple(data, i)
}
toc()
...
bias_quantification <- function(true_IRR_E, true_IRR_C, result_table)
{
  ...
  percent_bias_b1 <- abs(bias_beta_1)/true_beta1
  ...
  performance <- data.frame(missing_beta1, bias_beta_1, percent_bias_b1, coverage_irr_E, bias_beta_2, coverage_irr_C)
...
export(results_list, here("SCCS_Simulation","Results", "SCCS_results_confounder_1000_daily.csv"))

SCCS_confound_1000 <- import(here("Results", "SCCS_results_confounder_1000_daily.csv"))

bias_quantification_result <-bias_quantification(true_IRR_E=2.7, true_IRR_C=5, result_table = SCCS_confound_1000)
bias_quantification_result <- bias_quantification_result %>%
  mutate(mean_nr_event = mean(results_list$n_event))
bias_quantification_result
```

## 5. Confouder afftected by past exposure

### Function: Generate data

-   Cohort of 100,000 individual

-   Daily odds of the binary time-varying confounder C (COVID-19 infection): $10^{-3}$. Being infected in the previous 90 days reduce that odds by 70%. Being vaccinated in the previous 90 days reduce that odds by 80%. Based on the daily probability, generate the binary time-varying confounder using a Bernoulli trial.
...
```{r}
cohort_time_var_past_e <-function(n = 100000, obs_time = 500, risk_window = chosen_risk_window, 
                               p_C = 1e-3, C_reduce_C = 0.3, E_reduce_C = 0.2,
                               p_E = 3.2e-3, C_reduce_E = 0.25, E_reduce_E = 0.001,
                               baseline_Y = 2e-5, IRR_E = 2, IRR_C = 5)
{
  log_p_C <- log(p_C)
...
roll_E_onY <- integer(n)
  ...
  for (t in 1:obs_time) {
# Remove C or E events that fall outside the look-back window (28 days for C on E, C on Y, E on Y, and 91 days for C on C and E on C)
    if (t > 28) {
      roll_C_onE <- roll_C_onE - C[, t- 28]
    }
    if (t > risk_window) {
    ...
    # Generate C (depends on C[t-90, t-1] and E[t-90, t-1])
    recent_C_onC <- roll_C_onC > 0 # Check recent history of C
    recent_E_onC <- roll_E_onC > 0 # Check recent history of E
    prob_C <- plogis(log_p_C + log_C_reduce_C*recent_C_onC + log_E_reduce_C*recent_E_onC)
    C_t <- rbinom(n, 1, prob_C)
    C[, t] <- C_t
...
```{r}
set.seed(17)
tic("Cohort simulation, confounder by past exposure") # 22 sec
chosen_risk_window <- 28
test <- cohort_time_var_past_e(n = 100000, obs_time = 500, risk_window = chosen_risk_window, 
                               p_C = 1e-3, C_reduce_C = 0.3, E_reduce_C = 0.2,
                               p_E = 3.2e-3, C_reduce_E = 0.25, E_reduce_E = 0.001,
                               baseline_Y = 2e-5, IRR_E = 2, IRR_C = 5) 
toc()
...
time_var_past_30 <- foreach( i = 1:n_sim, .combine="rbind") %do% {
  rng_states[[i]] <- .Random.seed  # save RNG state
  data <- cohort_time_var_past_e(n = 100000, obs_time = 500, risk_window = chosen_risk_window, 
                               p_C = 1e-3, C_reduce_C = 0.3, E_reduce_C = 0.2,
                               p_E = 3.2e-3, C_reduce_E = 0.25, E_reduce_E = 0.001,
                               baseline_Y = 2e-5, IRR_E = 2.7, IRR_C = 6)
  data_SCCS <- SCCS_reformat_confound(data)
  result<-analyse_sccs_confound(data= data_SCCS, rep = i)
  p(sprintf("Iteration %d", i))  # update progress
  result
...
bias_quantification_result <-bias_quantification(true_IRR_E = 2.7, true_IRR_C = 6, result_table = time_var_past_30)
bias_quantification_result <- bias_quantification_result %>%
  mutate(mean_nr_event = mean(time_var_past_30$n_event))
bias_quantification_result
```

Simple model without adjustment for confounder
...
time_var_past_30 <- foreach( i = 1:n_sim, .combine="rbind") %do% {
  ...
  data <- cohort_time_var_past_e(n = 100000, obs_time = 500, risk_window = chosen_risk_window, 
                               p_C = 1e-3, C_reduce_C = 0.3, E_reduce_C = 0.2,
                               p_E = 3.2e-3, C_reduce_E = 0.25, E_reduce_E = 0.001,
                               baseline_Y = 2e-5, IRR_E = 2.7, IRR_C = 8)
  data_SCCS <- SCCS_reformat_confound(data)
...
bias_quantification_result <-bias_quantification(true_IRR_E = 2.7, true_IRR_C = 8, result_table = time_var_past_30)
bias_quantification_result <- bias_quantification_result %>%
  mutate(mean_nr_event = mean(time_var_past_30$n_event))
bias_quantification_result
```
...
-   Confounder U of C and Y: Daily odds $10^{-3}$. Duration of effect on C and Y: 90 days
...
# Remove U, C or E events that fall outside the look-back window (28 days for C on E, C on Y, E on Y, and 91 days for U on C, U on Y, C on C and E on C)
    ...
    # Generate Y (depends on U[t-89, t] C[t-risk_window+1, t], E[t-risk_window+1, t])
...
tic("SCCS with time-varying confounder and U")
rng_states <- list() #Store the state of the random number generator 

with_progress({
  p <- progressor(steps = n_sim)

time_var_past_U_30 <- foreach( i = 1:n_sim, .combine="rbind") %do% {
  rng_states[[i]] <- .Random.seed  # save RNG state
...
bias_quantification_result <-bias_quantification(true_IRR_E = 2.7, true_IRR_C = 6, result_table = time_var_past_U_30)
bias_quantification_result <- bias_quantification_result %>%
  mutate(mean_nr_event = mean(time_var_past_U_30$n_event))
bias_quantification_result
```
